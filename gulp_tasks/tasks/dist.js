...
var taskJSON = function(constants) {
...
gulp.task('json', function() {
	...
	return gmux.createAndRunTasks(gulp, taskJSON, taskname, global.options.target, global.options.mode, constants);
});

var taskJSONWatch = function(constants) {
	gulp.watch(constants.json.src, ['json']);
};

gulp.task('json:watch', 'Watch changes for json files.', function() {

	var taskname = 'json:watch';
	gmux.targets.setClientFolder(constants.clientFolder);
	if (global.options === null) {
		global.options = gmux.targets.askForMultipleTargets(taskname);
	}
	gmux.createAndRunTasks(gulp, taskJSONWatch, taskname, global.options.target, global.options.mode, constants);
});
